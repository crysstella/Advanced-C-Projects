 ...
 *
 */
std::string myDate::monthToString() {
	switch (month) {
	case 1:
		return "January ";
	case 2:
...
/**
 * Retrieve the date with format: January 1, 2000
 *
 * @return the date with format: January 1, 2000
 */
std::string myDate::getDate(){
	return monthToString() + to_string(getDay()) + ", " + to_string(getYear());
}